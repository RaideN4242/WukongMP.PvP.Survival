...
{
    // Добавляем флаг для отключения завершения раунда
    private bool _isSurvivalMode = true; // Установите в true для Survival режима
    
    protected override void OnUpdate(UpdateTick tick)
    {
        // Если включен Survival режим - пропускаем всю логику завершения раунда
        if (_isSurvivalMode)
            return;
            
        // Оригинальная логика (будет работать только если _isSurvivalMode = false)
